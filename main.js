...
if (typeof gsap !== 'undefined') {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // 00. Assortment Showcase (The Fan Reveal)
        if (window.innerWidth > 1024 && document.querySelector(".assortment-showcase")) {
            ...
            // 1. Initial State
            gsap.set(".pack-main", { y: 100, opacity: 0 });
            gsap.set(".pack-left, .pack-right", { x: 0, opacity: 0, scale: 0.8 });
            gsap.set(".feature-item-v2", { opacity: 0, x: 50 });

            // 2. Step 1: Packs fan out
            assortTl.to(".pack-main", { y: 0, opacity: 1, scale: 2.3, duration: 0.8, ease: "power3.out" })
                .to(".pack-left", { x: -160, rotate: -15, opacity: 0.9, scale: 1.3, duration: 1, ease: "power3.out" }, "-=0.5")
                .to(".pack-right", { x: 160, rotate: 15, opacity: 0.9, scale: 1.3, duration: 1, ease: "power3.out" }, "-=0.8");

            // 3. Step 2: Features on the right reveal with stagger
            assortTl.to(".feature-item-v2", { 
                opacity: 1, 
                x: 0, 
                duration: 0.8, 
                stagger: 0.2,
        ...
        if (document.querySelector(".hero-luxury-wrap")) {
            ...
            gsap.set(".main-lux-img", { scale: 0.7, y: 80 });
            ...
            gsap.set(heroImgs, { opacity: 0, x: 0, scale: 0.9, pointerEvents: "none" });
            const firstImg = document.querySelector('.main-lux-img.active');
            if (firstImg) {
                gsap.set(firstImg, { opacity: 1, scale: 1, x: 0, pointerEvents: "auto" });
        ...
        if (document.querySelectorAll('.animate-up, .animate-left, .animate-right').length > 0) {
            gsap.from(".animate-up", { y: 40, opacity: 0, duration: 1, ease: "power2.out", stagger: 0.1 });
            gsap.from(".animate-left", { x: -30, opacity: 0, duration: 1, delay: 0.2, ease: "power2.out" });
            gsap.from(".animate-right", { x: 30, opacity: 0, duration: 1, delay: 0.2, ease: "power2.out" });
        }

        // --- Swiper Initializations ---
        if (typeof Swiper !== 'undefined') {
            // --- Quality Swiper (Voice of Quality) ---
        ...
        if (catalogViewport && packTabsContainer && catalogGrid) {
            ...
            function renderGrid(packId, brand) {
                ...
                filtered.forEach((pd, idx) => {
                    ...
                    gsap.fromTo(item,
                        { opacity: 0, y: 30 },
                        { opacity: 1, y: 0, duration: 0.5, delay: idx * 0.05, ease: "back.out(1.5)" }
                    );
...
function switchMap(type) {
    ...
    if (type === 'google') {
        ...
        document.querySelector('[onclick="switchMap(\'google\')"]').classList.add('active');
    } else {
        googleMap.style.display = 'none';
        yandexMap.style.display = 'block';
        document.querySelector('[onclick="switchMap(\'yandex\')"]').classList.add('active');
    }
}
...
document.addEventListener('click', (e) => {
    ...
    if (!lang) return;
    setLanguage(lang);
    // Sync ALL lang buttons on the page
