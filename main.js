...
if (typeof gsap !== 'undefined') {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (document.querySelector(".assortment-showcase")) {
            // Desktop Layout (side-by-side) - compact fan-out
            ...
            // Mobile/Tablet Layout (stacked) - single main pack popping
            mm.add("(max-width: 1200px)", () => {
                ...
                gsap.set(".pack-left, .pack-right", { display: "none" });
        ...
        if (document.querySelector(".hero-luxury-wrap")) {
            ...
            if (isMobile) {
                gsap.set(".lux-word", { y: 30 });
                gsap.set(".luxury-badge, .luxury-lead, .luxury-cta-wrap", { y: 15, x: 0 });
                gsap.set(".main-lux-img", { scale: 1, y: 0, x: 0 });
            } else {
                gsap.set(".lux-word", { y: 100 });
                gsap.set(".luxury-badge, .luxury-lead, .luxury-cta-wrap", { x: -30, y: 0 });
                gsap.set(".glass-element", { scale: 0 });
                gsap.set(".main-lux-img", { scale: 0.7, y: 0, xPercent: -50, yPercent: -50 });
            }
            ...
            if (!isMobile) {
                // 2. DROPLETS SECOND (starting during text) - Desktop only
                luxTl.to(".glass-element", {
                    scale: 1,
                    opacity: 1,
                    duration: 0.8,
                    stagger: 0.03,
            ...
            if (isMobile) {
                gsap.set(heroImgs, { opacity: 0, pointerEvents: "none" });
                const firstImg = document.querySelector('.main-lux-img.active');
                if (firstImg) {
                    gsap.set(firstImg, { opacity: 1, pointerEvents: "auto" });
                    heroImgs.forEach((img, idx) => {
                        if (img.classList.contains('active')) currentHeroIdx = idx;
                    });
                }
            } else {
                gsap.set(heroImgs, { opacity: 0, x: 0, y: 0, xPercent: -50, yPercent: -50, scale: 0.9, pointerEvents: "none" });
                const firstImg = document.querySelector('.main-lux-img.active');
                if (firstImg) {
                    gsap.set(firstImg, { opacity: 1, scale: 1, x: 0, y: 0, xPercent: -50, yPercent: -50, pointerEvents: "auto" });
                    heroImgs.forEach((img, idx) => {
                        if (img.classList.contains('active')) currentHeroIdx = idx;
                    });
                }
            ...
            function cycleHero() {
                ...
                if (isMobile) {
                    // 1. Hide current product (Fade only on mobile)
                    tl.to(prevImg, {
                        opacity: 0,
                        duration: 0.6,
                        ease: "power2.inOut",
                    ...
                    // 2. Reveal next product (Fade only on mobile)
                    tl.to(nextImg, {
                        opacity: 1,
                        duration: 0.6,
                        ease: "power2.inOut",
                    ...
                    tl.to(prevImg, {
                        ...
                        onComplete: () => {
                            ...
                            gsap.set(prevImg, { x: 0, xPercent: -50, yPercent: -50 });
                        }
                    });

                    // 2. Reveal next product
                    tl.fromTo(nextImg,
                        { opacity: 0, x: 60, y: 0, xPercent: -50, yPercent: -50, scale: 1.1 },
                        { 
                            opacity: 1, 
                            x: 0, 
                            y: 0, 
        ...
        // Products Page Brand Banners Entrance Animation
        if (document.querySelector(".brand-banners-grid")) {
            const mmProducts = gsap.matchMedia();
            
            // Desktop/Tablet layout
            mmProducts.add("(min-width: 769px)", () => {
                const productsTimeline = gsap.timeline();
                const hasNav = document.querySelector(".category-nav");
                if (hasNav) {
                    productsTimeline.from(".category-nav", {
                        opacity: 0,
                        y: -15,
                        duration: 0.8,
                        ease: "power2.out",
                ...
                productsTimeline.from(".showcase-title", {
                    opacity: 0,
                    y: 15,
                    duration: 0.8,
                    ease: "power2.out",
            ...
            // Mobile layout (Optimized: smaller shifts, no scale modifications to prevent text/image re-rasterization lag)
            mmProducts.add("(max-width: 768px)", () => {
                const productsTimeline = gsap.timeline();
                const hasNav = document.querySelector(".category-nav");
                if (hasNav) {
                    productsTimeline.from(".category-nav", {
        ...
        if (catalogViewport && packTabsContainer && catalogGrid) {
            ...
            const ecoProducts = [
                ...
                { brand: 'eco', pack: '120', subcat: '', product: 'Aroma', img: 'img/ECO/120/aroma-120.png', color: 'linear-gradient(135deg, #e0f7fa 0%, #80deea 100%)' },
                ...
                { brand: 'eco', pack: '72', subcat: '', product: 'Aroma', img: 'img/ECO/72/aroma-72.png', color: 'linear-gradient(135deg, #e0f7fa 0%, #80deea 100%)' },
                ...
                { brand: 'eco', pack: '20', subcat: '', product: 'Aroma', img: 'img/ECO/20/aroma-20.png', color: 'linear-gradient(135deg, #e0f7fa 0%, #80deea 100%)' },
                ...
                { brand: 'eco', pack: '15', subcat: '', product: 'Aroma', img: 'img/ECO/15/aroma-15.jpg', color: 'linear-gradient(135deg, #e0f7fa 0%, #80deea 100%)' },
                ...
                { brand: 'eco', pack: '10', subcat: 'Compact', product: 'Compact', img: 'img/ECO/10/compact-10.jpg', color: 'linear-gradient(135deg, #ffebee 0%, #ff8a80 100%)' },
            ...
            const bloomyProducts = [
                ...
                { brand: 'bloomy', pack: '120', product: 'Magic', img: 'img/BLOOMY/120/iiii.webp', color: 'linear-gradient(135deg, #e1f5fe 0%, #b3e5fc 100%)' },
                ...
                { brand: 'bloomy', pack: '120', product: 'Tom Ford', img: 'img/BLOOMY/120/tomford-120.png', color: 'linear-gradient(135deg, #d4ecea 0%, #a8d8d2 100%)' },
                { brand: 'bloomy', pack: '120', product: 'Antibacterial Blue', img: 'img/BLOOMY/120/antibacterial-blue-120.png', color: 'linear-gradient(135deg, #e1f5fe 0%, #b3e5fc 100%)' },
                { brand: 'bloomy', pack: '120', product: 'Baby Boy', img: 'img/BLOOMY/120/baby-boy-120.png', color: 'linear-gradient(135deg, #ede7f6 0%, #d1c4e9 100%)' },
                { brand: 'bloomy', pack: '120', product: 'Baby Girl', img: 'img/BLOOMY/120/baby-girl-120.png', color: 'linear-gradient(135deg, #fbe4e4 0%, #f5c6cb 100%)' },
                { brand: 'bloomy', pack: '120', product: 'Excite', img: 'img/BLOOMY/120/excite-120.png', color: 'linear-gradient(135deg, #eceff1 0%, #b0bec5 100%)' },
                { brand: 'bloomy', pack: '120', product: 'Nature', img: 'img/BLOOMY/120/nature-120.png', color: 'linear-gradient(135deg, #fce4ec 0%, #f8bbd0 100%)' },
                // 72
                ...
                { brand: 'bloomy', pack: '72', product: 'Magic', img: 'img/BLOOMY/72/c_c_c.webp', color: 'linear-gradient(135deg, #e1f5fe 0%, #b3e5fc 100%)' },
                ...
                { brand: 'bloomy', pack: '20', product: 'Baby Pink', img: 'img/BLOOMY/20/cscsc.webp', color: 'linear-gradient(135deg, #e1bee7 0%, #ba68c8 100%)' },
                { brand: 'bloomy', pack: '20', product: 'Antibacterial', img: 'img/BLOOMY/20/fbfdb.webp', color: 'linear-gradient(135deg, #e1f5fe 0%, #b3e5fc 100%)' },
                { brand: 'bloomy', pack: '20', product: 'Magic', img: 'img/BLOOMY/20/lc.webp', color: 'linear-gradient(135deg, #e1f5fe 0%, #b3e5fc 100%)' },
                { brand: 'bloomy', pack: '20', product: 'Red Rose', img: 'img/BLOOMY/20/vvdvd.webp', color: 'linear-gradient(135deg, #ffebee 0%, #ffcdd2 100%)' },
                // 10
                { brand: 'bloomy', pack: '10', product: 'Antibacterial', img: 'img/BLOOMY/10/gfxhg.webp', color: 'linear-gradient(135deg, #ffebee 0%, #ff8a80 100%)' },
                { brand: 'bloomy', pack: '10', product: 'Magic', img: 'img/BLOOMY/10/hgjklk.webp', color: 'linear-gradient(135deg, #e1f5fe 0%, #b3e5fc 100%)' },
            ...
            const ayvaProducts = [
                // 120
                { brand: 'ayva', pack: '120', product: 'Family', img: 'img/AYVA/120/ayva-family-green-120.png', color: 'linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%)' },
                { brand: 'ayva', pack: '120', product: 'Family Garden', img: 'img/AYVA/120/ayva-family-120.png', color: 'linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%)' },
                { brand: 'ayva', pack: '120', product: 'Antibacterial', img: 'img/AYVA/120/ayva-antibaterial-120.png', color: 'linear-gradient(135deg, #e1f5fe 0%, #b3e5fc 100%)' },
                { brand: 'ayva', pack: '120', product: 'Antibacterial Green', img: 'img/AYVA/120/ayva-antibaterial-green-120.png', color: 'linear-gradient(135deg, #e8f5e9 0%, #c8e6c9 100%)' },
                { brand: 'ayva', pack: '120', product: 'Baby Aqua', img: 'img/AYVA/120/ayva-baby-aqua-120.png', color: 'linear-gradient(135deg, #e0f7fa 0%, #80deea 100%)' },
                { brand: 'ayva', pack: '120', product: 'Baby Honey', img: 'img/AYVA/120/ayva-baby-honney-120.png', color: 'linear-gradient(135deg, #fff9c4 0%, #fff59d 100%)' }
            ].map(p => {
                p.subcat = '';
                p.title = 'AYVA ' + p.product;
                p.spec = p.pack + ' шт';
                p.desc = `Влажные салфетки AYVA ${p.product} в удобной упаковке на ${p.pack} шт. Плотный материал, приятный аромат и бережный уход.`;
                p.specs = [
                    { icon: 'package', key: 'spec_pcs', val: p.pack },
                    { icon: 'layers', label: (parseInt(p.pack) >= 72) ? 'Спанлейс 40г/м2' : 'Спанлейс 35г/м2' },
                    { icon: 'droplet', key: 'spec_water', prefix: '99% ' },
            ...
            let allProducts = [...ecoProducts, ...bloomyProducts, ...ayvaProducts];

            const comingSoon = document.getElementById('coming-soon');
            const csBackBtn = document.querySelector('.cs-back-btn');
            const catNavBtn = document.querySelectorAll('.cat-nav-btn');

            const brandBannersSection = document.getElementById('brand-showcase');
            const compactBrandBar = document.getElementById('compact-brand-bar');
            const brandBannerCards = document.querySelectorAll('.brand-banner-card');
            const compactBrandBtns = document.querySelectorAll('.compact-brand-btn');

            const mysteryIntrigueModal = document.getElementById('mystery-intrigue-modal');
            const btnCloseMystery = document.getElementById('btn-close-mystery');
            const mysterySubscribeForm = document.getElementById('mystery-subscribe-form');
            const mysterySuccessBox = document.getElementById('mystery-success-box');
            ...
            function renderGrid(packId, brand) {
                ...
                filtered.forEach((pd, idx) => {
                    ...
                    if (pd.isDark) {
                        item.classList.add('is-dark-card');
                    }
            ...
            function openProductModal(product) {
                ...
                const descKey = 'desc_' + product.product.toLowerCase().replace(/ /g, '_');
                const prodDesc = translations[lang][descKey] || translations[lang]['prod_desc_standard'] || product.desc;
                const specSuffix = translations[lang]['spec_pcs'] || 'шт';
            ...
            // --- Brand Switcher & Showcase Logic --- //
            function selectBrand(brandId) {
                currentBrand = brandId;
                if (brandId !== 'ayva') {
                    previousBrand = brandId;
                }
                
                // Update active state on compact buttons
                compactBrandBtns.forEach(btn => {
                ...
                // Collapse banners and show compact bar + catalog
                if (brandBannersSection && brandBannersSection.style.display !== 'none') {
                    const tl = gsap.timeline();
                    tl.to(brandBannersSection, {
                        opacity: 0,
                        y: -30,
                        duration: 0.5,
                        ease: "power2.inOut",
                        onComplete: () => {
                            brandBannersSection.style.display = 'none';
                        }
                    });
                    
                    gsap.set(compactBrandBar, { display: 'block', height: 'auto', pointerEvents: 'all' });
                    tl.fromTo(compactBrandBar, 
                        { opacity: 0, y: -20 },
            ...
            // Click listeners for Large Banners
            brandBannerCards.forEach(card => {
            ...
            // Click listeners for Compact Switcher Buttons
            compactBrandBtns.forEach(btn => {
                btn.addEventListener('click', () => {
                    const brand = btn.getAttribute('data-filter');
                    selectBrand(brand);
                });
            });

            // --- Mystery Intrigue Modal Logic --- //
            let isMysteryAnimating = false;

            function openMysteryModal() {
                if (isMysteryAnimating || !mysteryIntrigueModal) return;
                isMysteryAnimating = true;

                // Reset modal state
                const lockIconWrap = document.getElementById('mystery-lock-icon-wrap');
                const lockIconSvg = document.getElementById('mystery-lock-icon-svg');
                if (lockIconWrap) lockIconWrap.classList.remove('unlocked');
                if (lockIconSvg) lockIconSvg.setAttribute('data-lucide', 'lock');
                if (typeof lucide !== 'undefined') lucide.createIcons();

                if (mysterySubscribeForm) {
                    mysterySubscribeForm.style.display = 'flex';
                    mysterySubscribeForm.style.opacity = '1';
                    mysterySubscribeForm.style.transform = 'translateY(0px)';
                    mysterySubscribeForm.reset();
                }
                if (mysterySuccessBox) {
                    mysterySuccessBox.style.display = 'none';
                    mysterySuccessBox.style.opacity = '0';
                }
                ...
                mysteryIntrigueModal.style.display = 'flex';
                mysteryIntrigueModal.style.pointerEvents = 'all';

                gsap.fromTo(mysteryIntrigueModal,
                    { opacity: 0, backdropFilter: 'blur(0px)' },
                    { opacity: 1, backdropFilter: 'blur(25px)', duration: 0.5, onComplete: () => { isMysteryAnimating = false; } }
                );

                gsap.fromTo('.mystery-modal-box',
            ...
            function closeMysteryModal() {
                ...
                mysteryIntrigueModal.style.pointerEvents = 'none';

                gsap.to('.mystery-modal-box', { y: 40, opacity: 0, scale: 0.96, duration: 0.35, ease: "power2.in" });
                gsap.to(mysteryIntrigueModal, {
                    opacity: 0,
                    backdropFilter: 'blur(0px)',
                    duration: 0.35,
                    onComplete: () => {
                        mysteryIntrigueModal.style.display = 'none';
                        document.body.style.overflow = '';
                        document.body.style.paddingRight = '';
                        const header = document.querySelector('app-header');
                        if (header) header.style.paddingRight = '';
                        isMysteryAnimating = false;
                    }
                });
            }

            if (btnCloseMystery) {
                btnCloseMystery.addEventListener('click', (e) => {
                    e.stopPropagation();
                    closeMysteryModal();
                });
            }

            if (mysteryIntrigueModal) {
                mysteryIntrigueModal.addEventListener('click', (e) => {
                    if (e.target === mysteryIntrigueModal) closeMysteryModal();
                });
            }

            // Escape key to close mystery modal
            window.addEventListener('keydown', (e) => {
                if (e.key === 'Escape' && mysteryIntrigueModal && mysteryIntrigueModal.style.display === 'flex') {
                    closeMysteryModal();
                }
            });

            // Form Submit Logic (Notify Me)
            if (mysterySubscribeForm) {
                mysterySubscribeForm.addEventListener('submit', (e) => {
                    e.preventDefault();
                    
                    const inputVal = document.getElementById('mystery-contact-input').value.trim();
                    if (!inputVal) return;

                    // Animate form fading out
                    gsap.to(mysterySubscribeForm, {
                        opacity: 0,
                        y: -10,
                        duration: 0.3,
                        onComplete: () => {
                            mysterySubscribeForm.style.display = 'none';
                            ...
                            if (lockIconWrap) {
                                lockIconWrap.classList.add('unlocked');
                                
                                // Lucide lock to unlock icon change
                                if (lockIconSvg) {
                                    lockIconSvg.setAttribute('data-lucide', 'unlock');
                                    if (typeof lucide !== 'undefined') lucide.createIcons();
                                }
                            }
                            ...
                            if (mysterySuccessBox) {
                                mysterySuccessBox.style.display = 'flex';
                                gsap.fromTo(mysterySuccessBox,
            ...
            function showComingSoonOverlay() {
                ...
                if (brandBannersSection) {
                    gsap.to(brandBannersSection, {
            ...
            catNavBtn.forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    if (cat === 'wipes') {
                        // Restore correct brand selector state
                        if (currentBrand) {
                            if (compactBrandBar) {
                                compactBrandBar.style.display = 'block';
                                gsap.to(compactBrandBar, { height: 'auto', opacity: 1, pointerEvents: 'all', duration: 0.5 });
                            }
                        } else {
                            if (brandBannersSection) {
                                brandBannersSection.style.display = 'block';
                                gsap.to(brandBannersSection, { height: 'auto', opacity: 1, pointerEvents: 'all', duration: 0.5 });
                            }
                        }
            ...
            if (csBackBtn) {
                csBackBtn.addEventListener('click', () => {
                    if (previousBrand) {
                        selectBrand(previousBrand);
                    } else {
                        // Go back to the main banners
                        currentBrand = null;
                        if (brandBannersSection) {
                            brandBannersSection.style.display = 'block';
                            gsap.to(brandBannersSection, { height: 'auto', opacity: 1, pointerEvents: 'all', duration: 0.5 });
                        ...
                        if (compactBrandBar) {
                            gsap.to(compactBrandBar, {
                                height: 0, opacity: 0, pointerEvents: 'none', duration: 0.4, onComplete: () => {
                                    compactBrandBar.style.display = 'none';
                                }
                            });
                        }
